...
export default function Home() {
  ...
  return (
    <main className="min-h-screen w-full bg-[#FAFAFC] text-slate-900 pb-32 overflow-x-hidden">
      ...
      <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200/60">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 h-16 sm:h-20 flex items-center justify-between w-full">
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl sm:rounded-2xl bg-slate-900 flex items-center justify-center shadow-md shadow-slate-900/10">
          ...
          {/* Desktop Navigation (auf Mobile ausgeblendet) */}
          <div className="hidden md:flex items-center gap-3">
            <Link 
              href="/neu" 
              className="bg-slate-900 hover:bg-slate-800 text-white font-semibold px-5 py-2.5 rounded-2xl transition-all text-sm flex items-center gap-2"
            >
              <span>✨</span> Neues Rezept
            </Link>
            <Link 
              href="/manuell" 
              className="bg-white hover:bg-slate-50 text-slate-700 font-semibold px-5 py-2.5 rounded-2xl border border-slate-200/80 transition-all text-sm flex items-center gap-2"
            >
      ...
      {/* Hero Section */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-8 sm:pt-16 pb-8 w-full">
        <div className="max-w-2xl">
          ...
          <h1 className="text-3xl sm:text-5xl font-black tracking-tight text-slate-900 mb-3 leading-tight">
        ...
        <div className="mt-6 sm:mt-8 max-w-xl w-full">
          <div className="relative">
            ...
            <input 
              ...
              className="w-full pl-11 pr-4 py-3.5 sm:py-4 bg-white rounded-2xl border border-slate-200/80 shadow-sm outline-none focus:border-slate-900 text-slate-900 placeholder:text-slate-400 font-medium text-sm sm:text-base"
            />
          </div>
        </div>

        {/* Category Filter Pills */}
        <div className="flex gap-2 overflow-x-auto pt-4 sm:pt-6 pb-2 no-scrollbar w-full">
          {categories.map((cat) => (
            <button
              key={cat}
              onClick={() => setSelectedCategory(cat)}
              className={`px-4 sm:px-5 py-2 sm:py-2.5 rounded-xl sm:rounded-2xl font-semibold text-xs sm:text-sm whitespace-nowrap transition-all ${
            ...
            {filteredRecipes.map((recipe) => {
              ...
              return (
                <Link 
                  ...
                  key={recipe.id}
                  className="group bg-white rounded-3xl border border-slate-200/70 overflow-hidden shadow-sm hover:shadow-xl transition-all duration-500 flex flex-col w-full"
                >
                  <div className="relative h-48 sm:h-56 w-full overflow-hidden bg-slate-100">
                    ...
                    />
                    <div className="absolute top-3 right-3 sm:top-4 sm:right-4 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold text-slate-800 shadow-sm border border-white/25 z-10">
                      {badgeText}
                    </div>
                  </div>
                  ...
                  <div className="p-4 sm:p-6 flex flex-col flex-grow justify-between bg-white">
                    <div>
                      <h2 className="text-lg sm:text-xl font-extrabold text-slate-900 group-hover:text-slate-600 transition-colors mb-1 sm:mb-2 line-clamp-1">
                        {recipe.title}
                      </h2>
                    ...
                    <div className="mt-4 sm:mt-6 pt-3 sm:pt-4 border-t border-slate-100 flex items-center justify-between text-xs sm:text-sm font-bold text-slate-900">
